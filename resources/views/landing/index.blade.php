...
<div id="landing-root">
    <div id="sidebar-nav" class="fixed left-0 top-0 z-50 h-screen w-[130px] px-3 py-6 xl:w-[200px] transition-transform duration-500">
        <div class="flex flex-col gap-6">
            ...
            <nav class="font-pragextobl flex flex-col items-start gap-3">
                ...
                <a
                    href="#{{ $item['id'] }}"
                    class="nav-item js-nav-item group relative flex h-10 items-center justify-center text-[#FF4F00] text-bold"
                    data-target="{{ $item['id'] }}">
                    <span class="nav-icon flex h-12 w-12 items-center justify-center sm:h-12 sm:w-12 lg:h-13 lg:w-13 xl:h-14 xl:w-14">
                        <span class="h-3/4 w-3/4 rounded-full border-2 border-[#FF4F00] sm:h-7 sm:w-7 lg:h-8 lg:w-8 xl:h-9 xl:w-9"></span>
                    </span>
    ...
    <main class="relative">
        ...
        <div id="frames-wrapper" class="relative h-screen w-screen overflow-hidden bg-[#FF4F00]">
            ...
            <section
                ...
                <div class="pointer-events-none absolute inset-0 flex flex-col justify-end items-center pb-16 px-6 md:justify-center md:items-end md:pb-0 md:pr-16 lg:pr-24">
                    <div class="js-frame-text w-full text-center max-w-[900px] md:text-right md:max-w-[70vw] lg:max-w-[80vw]">
                        @if($section?->title)
                        <h2 class="text-5xl md:text-7xl lg:text-9xl font-pragextobl text-[#FF4F00] mb-4 md:mb-6 lg:mb-8 break-words leading-tight">
                            {{ $section->title }}
                        </h2>
                        @endif
        ...
        <footer id="rodape" class="gallery-bg relative min-h-screen w-screen">
            <div class="px-6 pt-16 sm:pt-24 relative z-10">
                @if($rodapeTitulo)
                <h1 class="text-5xl md:text-6xl lg:text-7xl font-bold text-[#FF4F00] mb-4 font-pragextobl text-center">
                    <span class="line-wrap"><span class="line-inner footer-line" data-delay="0">{{ $rodapeTitulo }}</span></span>
                </h1>
                @endif

                @if($rodapeSubtitulo)
                <h4 class="text-lg md:text-xl font-pragext text-center text-[#FF4F00] mb-10">
            ...
            <div class="mx-auto mt-12 flex max-w-5xl flex-col items-center gap-6 px-6 pb-16 text-center relative z-10">
                <a href="#publicidade"
                    class="flip-logo-wrap block"
                    data-src="{{ asset(config('app.logo_path')) }}"
                ...
                <div class="flex flex-wrap items-center justify-center gap-6 text-sm">
                    <p class="font-pragext text-[#FF4F00]">
